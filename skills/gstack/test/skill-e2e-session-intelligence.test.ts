    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const full = fs.readFileSync(
          path.join(ROOT, "checkpoint", "SKILL.md"),
          "utf-8",
        );
        ...
        const saveSection = full.slice(
          saveStart,
          resumeStart > saveStart ? resumeStart : undefined,
        );
...
- Do NOT use AskUserQuestion.`,
        ...
        if (checkpointCreated) {
          checkpointContent = fs.readFileSync(
            path.join(checkpointDir, checkpointFiles[0]),
            "utf-8",
          );
        }
        ...
        const hasYamlFrontmatter =
          checkpointContent.includes("---") &&
          checkpointContent.includes("status:");
        const hasBranch =
          checkpointContent.includes("branch:") ||
          checkpointContent.includes("main");
