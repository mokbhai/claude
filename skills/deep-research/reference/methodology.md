...
**CRITICAL: Use SearXNG private search script for all web searches**

Use the bundled search script via Bash tool:

```bash
python3 ~/.claude/skills/deep-research/scripts/search.py "your query" --json --num 10
```

**SearXNG Search Features:**

- Aggregates results from multiple engines (Brave, DuckDuckGo, Startpage, Google, Wikipedia)
- Returns structured JSON with title, URL, and content snippets
- Private search with no tracking
- Supports `--json` for machine-readable output and `--num N` to limit results
...
**Example parallel execution (using SearXNG search):**

```
[Single message with multiple Bash tool calls + Task agents]
- Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing 2025 state of the art" --json --num 10
- Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing limitations challenges" --json --num 10
- Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing commercial applications 2024-2025" --json --num 10
- Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing vs classical comparison" --json --num 10
- Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum error correction research" --json --num 10
...
- Use SearXNG search script for all web searches (primary tool)
  ```bash
  python3 ~/.claude/skills/deep-research/scripts/search.py "query" --json --num 10
  ```
- Use fetch_url script for deep dives into specific URLs (extracts full content with metadata)
  ```bash
  python3 ~/.claude/skills/deep-research/scripts/fetch_url.py "https://example.com/article" --json
  ```

  - Special handling for Reddit (via JSON API, includes comments)
  - Special handling for Hacker News (via Firebase API)
  - Extracts clean text, metadata, and quality metrics
