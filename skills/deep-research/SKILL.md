...
2. **Launch ALL searches in single message** using SearXNG script via Bash tool (NOT sequential)
...
**Search Tool: Use SearXNG Private Search (REQUIRED)**

Use the bundled search script for all web searches:

```bash
python3 ~/.claude/skills/deep-research/scripts/search.py "your query" --json --num 10
```

The script returns JSON with structured results from multiple engines (Brave, DuckDuckGo, Startpage, Google, Wikipedia).

**URL Fetcher: Deep Content Extraction (REQUIRED for detailed source analysis)**

After finding relevant URLs via search, use the fetch_url script to extract full content:

```bash
python3 ~/.claude/skills/deep-research/scripts/fetch_url.py "https://example.com/article" --json
```

**Features:**

- Extracts clean text from HTML pages
- Special handling for Reddit (via JSON API with comments)
- Special handling for Hacker News (via Firebase API)
- **Automatic Puppeteer fallback** for JS-heavy/CAPTCHA sites (Medium, Substack, Quora, etc.)
- Metadata extraction (title, author, publish date, etc.)
- Quality metrics (word count, reading time)
- JSON output for easy parsing

**Browser Automation (Puppeteer):**
The fetcher automatically uses Puppeteer with stealth plugins when:

- Site is known to require JavaScript (Medium, Substack, Quora, Twitter, etc.)
- CAPTCHA or blocking is detected during initial fetch
- urllib fails with SSL or other errors

For manual Puppeteer usage:

```bash
cd ~/.claude/skills/deep-research/scripts
node puppeteer_fetch.js "https://medium.com/article" --json --timeout 60 --wait 3
```
...
python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing 2025" --json --num 10

# 2. Extract content from promising URLs
python3 ~/.claude/skills/deep-research/scripts/fetch_url.py "https://reddit.com/r/QuantumComputing/..." --json
python3 ~/.claude/skills/deep-research/scripts/fetch_url.py "https://arxiv.org/abs/..." --json
```

**Example correct execution:**

```
[Single message with multiple parallel Bash tool calls for searches]
Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing 2025" --json --num 10
Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing limitations" --json --num 10
Bash: python3 ~/.claude/skills/deep-research/scripts/search.py "quantum computing commercial applications" --json --num 10
...
```
Bash search #1 → wait for results → Bash search #2 → wait → Bash search #3...
```
...
- **search.py** - SearXNG private web search (aggregates Brave, DuckDuckGo, Startpage, Google, Wikipedia)
- **fetch_url.py** - URL content fetcher with special handling for Reddit, HN, and other JS-heavy sites
